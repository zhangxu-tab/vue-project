...
// 获取当前被编辑的商品信息
import {getSeckById} from '../../request/seckill';
...
export default {
  ...
  async created() {
    const res = await this.$http.all([
      getCateList(),
      getGoodsList(),
      getSeckById(this.formData.id)
      ]);
    if(res) {
      // 更新分类数组
      // 更新商品数组
      [this.firstCateList,this.goodsList,this.formData] = res;

      // 获取二级分类
      this.firstCateChange(this.formData.first_cateid);
      // 获取商品列表
      this.secondCateChange(this.formData.second_cateid);

      const bt = Number(this.formData.begintime);
      const et = Number(this.formData.endtime);
      const b1 = moment(bt).format('YYYY-MM-DD HH:mm:ss');
      const e1 = moment(et).format('YYYY-MM-DD HH:mm:ss');

      this.value.push(b1,e1);
    }
  },
  methods: {
    // 一级分类的监听事件
